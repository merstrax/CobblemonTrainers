...
public class TrainerPokemon {
    ...
    public TrainerPokemon(JsonElement jsonElement) {
        ...
        if (species == null) {
            throw new IllegalStateException("Invalid species: " + speciesString);
        } else {
            ...
            if (nature == null) {
                throw new IllegalStateException("Invalid nature: " + natureString);
            } else {
                ability = new Ability(Abilities.INSTANCE.getOrException(jsonObject.get(POKEMON_ABILITY).getAsString()), false, Priority.NORMAL);
                ...
                if (jsonObject.has(POKEMON_SHINY)) {
                    isShiny = jsonObject.get(POKEMON_SHINY).getAsBoolean();
                }

                if (jsonObject.has(POKEMON_HELD_ITEM)) {
                    heldItem = Registries.ITEM.get(Identifier.tryParse(jsonObject.get(POKEMON_HELD_ITEM).getAsString()));
                }
